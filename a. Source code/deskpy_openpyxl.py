    ...
    def sck_folder(self):
        try:
            ...
            for sub_folder in self.main_folder:
                ...
                for item in self.normalized_list:
                    if item.__contains__('analisis'): is_analysis = True
                        ...
                        if item.__contains__('cicac'): is_cicac = True
                            ...
                            if item.__contains__('cic'):
                                x = re.search('.{2}[-].{2}[-].{4}', item)
                                if x: is_cic_dated = True
                                else: is_cic = True
                                if item.__contains__('autorizac'): is_cic = True
                ...
                if not is_analysis: self.subf_dataset.append('ANÁLISIS')
                if not is_cic: self.subf_dataset.append('AUTORIZACIÓN CIC')
                if not is_cic_dated: self.subf_dataset.append('CIC')
                if not is_cicac: self.subf_dataset.append('CICAC')
                if not is_contract: self.subf_dataset.append('CONTRATO')
                if not is_affidavit: self.subf_dataset.append('DECLARACIÓN JURADA')
                if not is_id: self.subf_dataset.append('ID')
                if not is_kyc: self.subf_dataset.append('KYC')
                if not is_document: self.subf_dataset.append('PAGARÉ')
            ...
            for ts in to_style:
                for cell in ts:
                    cell.border = Border(top=self.wb_border, right=self.wb_border, bottom=self.wb_border, left=self.wb_border)
            os.system('cls')
            print(f'Expedientes procesados: {ltree}.\nEl programa ha finalizado.\n')
            wb.save(myexcel)
